...
def update_feedback(request):
    ...
    if request.method == 'POST':
        ...
        report_missed = True if int(
            request.POST['report_missed']) == 1 else False
        ...
        if(len(feedbacks) == 0):  # if not exist, create
            Feedback(case=case, comment=request.POST['comment'],
                     is_incorrect=not is_correct, is_difficult=is_difficult, report_missed=report_missed).save()
        else:
            feedbacks[0].comment = request.POST['comment']
            feedbacks[0].is_incorrect = not is_correct
            feedbacks[0].is_difficult = is_difficult
            feedbacks[0].report_missed = report_missed
            feedbacks[0].save()
