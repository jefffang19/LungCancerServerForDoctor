...
from patches_cls_with_cam import get_cls_model, fit
...
def run_inference(dcm_path, data_path, thresh=0.5, verbose=False):
    ...
    for image, ori, high_res, pat_id in dataset:
        ...
        prd, cams = fit(model, x)
        
        # classification result
        results = np.where(prd[:, 1] > thresh, 1, 0)
        
        # save cams
        os.mkdir(os.path.join(data_path, pat_id, 'cams'))
        for i in range(16):
            cv2.imwrite(os.path.join(data_path, pat_id,
                                     'cams', 'x{}.png'.format(i)), cv2.cvtColor(cams[i], cv2.COLOR_RGB2BGR))
            if verbose:
                print(cams[i].shape)
